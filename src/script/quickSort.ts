export function quickSort(list: number[], i = 0, j?: number): void {
  j ??= list.length - 1
  if (i >= j) return
 ...
 *
 */
function partition(list: number[], i: number, j: number): number {
 ...
 *
 */
function middleOfThree(a: number, b: number, c: number): number {
  if (a < b) {
    if (b < c) return b
